...
LRESULT CALLBACK MarkProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	LPWSTR Marktxt[3] = { (LPWSTR)TEXT("Color selection"), (LPWSTR)TEXT("Shape Selection"), (LPWSTR)TEXT("Buttons")};
	switch (message)
	{
	case WM_CREATE:
	{
		...
		if (bithsize == INVALID_FILE_SIZE || bithsize == 0)//file not present, fill it with basic values
		{
			...
			SafewriteFile(hFile, filebuffer, (DWORD)charsnumber, NULL, &overlapstructshit);
			free(filebuffer);
			filebuffer = NULL;
			SafeCloseHandle(hFile);
		}
		else//filepresent
			(void)marksfilepresent(bithsize, hFile);
		...
		for (int i = 0; i < 100; i++)
		{
			offset = (int)GetModuleFileNameA(NULL, directoryplace, 1000);
			...
			if (GetLastError() == 0)
				MyBitmapsArray[i] = (HBITMAP*)BitmapHandle;
			else
			{
	...
	case WM_COMMAND://???
	{
		...
		int filesize = (int)GetFileSize(MarksFile, NULL);
		...
		if (filesize > 16)//enough data for dataset is present
		{
			...
			Marksbuffer = (char*)SafeCalloc((size_t)filesize, sizeof(char));
			returnval = ReadFile(MarksFile, Marksbuffer, (DWORD)filesize, NULL, &overlapstruct);
			free(Marksbuffer);
			Marksbuffer = NULL;
			if (returnval == 0)
			{
...
HWND CreateButtonMk(LPWSTR szString, RECT Size, HWND hwnd, int index)
{
	...
	if (firsttime == TRUE)
	{
		if (0 == RegisterClassEx(&buttonclassmk))
		{
			CrashDumpFunction(2248, 0);
			return NULL;
		}
		firsttime = FALSE;
	}
	...
	if (hwndButtonmk == NULL)
	{
		CrashDumpFunction(2395, 0);
		return NULL;
	}
	RECT shutup = { 0 };
	...
	SafeSetWindowLongPtr(hwndButtonmk, GWLP_USERDATA, index);
	if (hwndButtonmk)
		return hwndButtonmk;
	else
	{
		CrashDumpFunction(2271,1);
		return NULL;
	}
}
...
LRESULT CALLBACK ButtonProcMk(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_PAINT:
	{
		...
		if (hdc == 0)
		{
			CrashDumpFunction(2297, 1);
			return 0;
		}
		HDC memdc = { 0 };
		hBrush = SafetCreateSolidBrush(RGB(222, 255, 255));
		assert(hBrush != INVALID_HANDLE_VALUE);
		...
		if (WindowIndex == 0)//shapes window
		{
			for (int i = (scrollamount[WindowIndex] * 2); i < 100 && MyBitmapsArray[i] != NULL; i++)
			{
				...
				if (0 == SetBrushOrgEx(hdc, 0, 0, NULL))
					CrashDumpFunction(2422, 0);
	...
	case TIMEUPDATE://timeupdateloop
	{
		...
		DrawTextASafe(hdc, mytimedata, -1, &LocalRect, DT_TOP | DT_LEFT);
	...
	case DATEUPDATETICK:
	{
		...
		SafeDeleteObject(hBrush);
		if (-1 == sprintf_s(memdata, 1000, "remaining bytes: \n\n%zu ", TextHeapRemaining))
		{
			CrashDumpFunction(50, 0);
		}
		SafeSetBkColor(hdc, RGB(200, 120, 80));
		SafeSetTextColor(hdc, RGB(0, 0, 255));
		DrawTextASafe(hdc, memdata, -1, &DataUpdateRect, DT_LEFT);
		//Data remaining
		memset(memdata, 0, sizeof(char) * 1000);
		...
		SafeFillRect(hdc, &LocalRect, hBrush);
		SafeDeleteObject(hBrush);
		if (-1 == sprintf_s(memdata, 1000, "avalible bytes: \n\n%lld", pchinputbuffermemory))
		{
			CrashDumpFunction(50, 0);
		}
		SafeSetBkColor(hdc, RGB(33, 200, 3));
		SafeSetTextColor(hdc, RGB(0, 0, 0));
		DrawTextASafe(hdc, memdata, -1, &LocalRect, DT_LEFT);
		SafeReleaseDC(hwnd, hdc);
		return 0;
	}
	case WM_LBUTTONDOWN:
	{
		...
		if (WindowIndex == 0)//shapes signaling
		{
			...
			(void)SendMessage(DatesHwnd, MARK_SIGNAL, (WPARAM)selecteddate, (LPARAM)indexselected);
		}
	...
	case WM_RBUTTONDOWN://right click means change color or shape of a given window
	{
		...
		if (WindowIndex == 1)//color change
		{
			...
			colordlg.lpCustColors = (COLORREF*)colorspointer,
			colordlg.Flags = CC_SOLIDCOLOR | CC_FULLOPEN,
			colordlg.lCustData = 0,
			colordlg.lpfnHook = NULL,
			colordlg.lpTemplateName = NULL
			};
			if (0 == ChooseColorA(&colordlg))
			{
				CrashDumpFunction(2494, 0);
			}
			int realindex = 0;
			int returnval = 0;
			returnval = (int)CommDlgExtendedError();
			if (returnval > 0)
			...
			OVERLAPPED overlapstruct = { 0 };
			char* filebuffer = (char*)SafeCalloc((size_t)(filesize + 1), sizeof(char));
			bool boolflag1 = ReadFile(hFile, filebuffer, (DWORD)filesize, NULL, &overlapstruct);
			if (boolflag1 == 0)
				CrashDumpFunction(120, 0);
			//append the data update to rgbdata
			RGBDatatxtUpdate(filebuffer, realindex, (int)selectedcolor, filesize, hFile, overlapstruct);
	...
	case WM_DESTROY:
		...
		if( 0 == DestroyButton(hwnd))
			CrashDumpFunction(2552, 0);
		return 0;
	}
	return DefWindowProc(hwnd, message, wParam, lParam);
}
...
{
	(void)ButtonProcMk(hwnd, TIMEUPDATE, 0, 0);
}
...
LRESULT CALLBACK TempBoxProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_KEYDOWN:
		...
		hdc = safeGetDC(hwnd);
		SelectFont(hdc, localfont);
		if(0 == GetTextExtentPoint32A(hdc, numberarray + index, (int)strlen(numberarray + index), &sizeextent))
		{
			CrashDumpFunction(2636, 0);
		}
		boxsize.right += sizeextent.cx;
		SafeReleaseDC(hwnd, hdc);
		if(0==CopyRect(&parentclient, &boxsize))
		{
			CrashDumpFunction(2637, 0);
		}
	...
	case WM_KILLFOCUS://apply change
		...
		if(GlobalIndex == 1)//yearchange
		{
			...
			if ((yearzero < year) && (yearzero + year-yearzero) < yearzero + yearrange / 12)//if its in scope of year-range
			{
				...
				{
					SafeSetWindowLongPtr(buttonarray[i], GWLP_USERDATA, zerothwindowpos);
				}
		...
		if (GlobalIndex == 2)//monthchange
		{
			...
			int month = atoi(numberarray)-1;
			int oldyear = datedate >> 11;
			if((month<12 && month >=0) && ((month + oldyear*12-yearzero*12)<yearrange))
			{
...
void marksfilepresent(int bithsize, HANDLE hFile)
{
	assert(hFile != INVALID_HANDLE_VALUE);
	OVERLAPPED overlapstruct = { 0 };
	char * filebuffer = (char*)SafeCalloc(  (size_t)bithsize,    sizeof(char));
	if (hFile == NULL)
		return;
	char* pEnd = NULL;
	int colorindex = 0;
	int rgbColor = 0;
	int firstpoint = 0;
	bool totaldeath = ReadFile(hFile, filebuffer, (DWORD)bithsize, NULL, &overlapstruct);
	...
	for (int i = 0; i < 100; i++)
	{
		for (int k = firstpoint; k < bithsize && k < 100000; k++)
		{
			if (filebuffer[k] == '=')
			{
				...
				colorindex = Safestrtol(filebuffer + firstpoint, &pEnd, 10);
				...
				rgbColor = Safestrtol(filebuffer + k, &pEnd, 10);
				ColorSelection[i] = (COLORREF)rgbColor;
				if (rgbColor > 9)
...
RECT buttonswindow(HWND hwnd, POINT mypt)
{
	...
	{//if you click live date paste the live data, put live date on clipboard
		HDC hdc = { 0 };
		hdc = safeGetDC(hwnd);
		if (FALSE == InvertRect(hdc, &TempRect2))
		{
			CrashDumpFunction(2781, 0);
		}
		SafeReleaseDC(hwnd, hdc);
		...
		cliptext = (char*)GlobalLock(clipmemory);
		if (cliptext == NULL)
		{
		...
		SYSTEMTIME lTime = { 0 };
		GetSystemTime(&lTime);
		if (FALSE == SystemTimeToTzSpecificLocalTime(NULL, &lTime, &lTime))
		{
			CrashDumpFunction(2807, 0);
		}
...
void DatesChangeWindow(HWND hwnd, POINT mypt, RECT TempRect2)
{
	...
	{//Year
		...
		hdc = safeGetDC(hwnd);
		SelectFont(hdc, localfont);
		if(FALSE == (GetTextExtentPoint32A(hdc, tempbuffer10, 6, &sizeextent)))
		{
			CrashDumpFunction(2875, 0);
		}
		SafeReleaseDC(hwnd, hdc);
		val = 1;
	...
	{//Month
		...
		hdc = safeGetDC(hwnd);
		SelectFont(hdc, localfont);
		if(0==GetTextExtentPoint32A(hdc, tempbuffer10, 7, &sizeextent))
		{
			CrashDumpFunction(2899, 0);
		}
		SafeReleaseDC(hwnd, hdc);
		val = 2;
		tempwindow = CreateWindowExA(0, "Tempclass", "TempBox", WS_CHILD | WS_VISIBLE, sizeextent.cx, TempRect2.top, sizeextent.cx, sizeextent.cy, hwnd, NULL, NULL, &val);
...
void ShapesChangeButton(int indexselected, HWND hwnd)
{
	...
	{
		CrashDumpFunction(2968, 0);
	}
	int offset = 0;
	offset = (int)GetModuleFileNameA(NULL, currentdirectory, 1000);
	if (offset == 0)
	{
		CrashDumpFunction(2974, 0);
	}
	...
	if(FALSE == CopyFileA(filenamestring, currentdirectory, FALSE))
	{
		CrashDumpFunction(2987, 0);
	}
	HANDLE BitmapHandle = { 0 };
	BitmapHandle = LoadImageA(NULL, filenamestring, IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE);//Load image to get BITMAP handle
	if (GetLastError() == 0)
	{
		MyBitmapsArray[indexselected] = (HBITMAP*)BitmapHandle;
	}
	else
		CrashDumpFunction(2997, 0);
	InvalidateRectSafe(SafeGetParent(TextBoxHwnd), NULL, TRUE);
	UpdateWindowSafe(TextBoxHwnd);
}
...
#pragma warning(pop)
		{
			...
			colorindex = Safestrtol(filebuffer + firstpoint, &endptr, 10);
			i++; //to skip equal sign
			if (colorindex == realindex)
			{//writeover
				//get old number
				rgbColor = Safestrtol(filebuffer + i, &endptr, 10);
				...
				if (totaldifference < 0)//shrink
				{
					...
					SafeSetFilePointer(hFile, totaldifference, NULL, FILE_END);
					SafeSetEndOfFile(hFile);//shrink the file accordingly
				}
				...
				overlapstruct.Offset = (DWORD)i;
#pragma warning(push)
#pragma warning(disable: 6001)
				if (hFile == INVALID_HANDLE_VALUE) return;
				SafewriteFile(hFile, filebuffer + i, (DWORD)(filesize - i + totaldifference), NULL, &overlapstruct);
				SafeCloseHandle(hFile);
#pragma warning(pop)
				free(filebuffer);
...
RECT ButtonsBarFunction(RECT LocalRect, HDC hdc, HWND hwnd, int * step, RECT DataUpdateRect)
{
	...
	//Live Date presented
	GetSystemTime(&sTime);
	if(0 == SystemTimeToTzSpecificLocalTime(NULL, &sTime, &sTime))
	{
		CrashDumpFunction(1106, 0);
	}
	...
	SafeSetBkColor(hdc, RGB(40, 40, 40));
	SafeSetTextColor(hdc, RGB(150, 150, 150));
	DrawTextASafe(hdc, mytimedata, -1, &TempRect, DT_TOP | DT_LEFT);
	...
	DrawTextASafe(hdc, mytimedata, -1, &TempRect, DT_LEFT);
	SafeFrameRect(hdc, &TempRect, frameBrush);
	if(FALSE == RestoreDC(hdc, -1))
	{
		CrashDumpFunction(1151, 0);
	}
...
RECT ButtonsDataWipeandBelow(char * mytimedata, RECT LocalRect, RECT TempRect, HDC hdc, HBRUSH frameBrush, int * step, RECT DataUpdateRect)
{
	...
	DrawTextASafe(hdc, mytimedata, -1, &TempRect, DT_CENTER | DT_LEFT);
	SafeFrameRect(hdc, &TempRect, frameBrush);
...
void ButtonsFunction(LPARAM lParam, HWND hwnd, int step)
{
	...
	if (PtInRect(&TempRect2, mypt))//data wipe
	{
		...
		if (MessageBox(SafeGetParent(DatesHwnd), TEXT("This will delete all data for the day, are you sure?"), TEXT("Data Wipe"), MB_OKCANCEL | MB_ICONWARNING) == IDOK)
		{
			...
			if (selecteddate != 0)
				if(FALSE == RangedDataWipe(month1, month1, year1, year1, day1, day1))//deletes only one day, the day selected.
				{
					CrashDumpFunction(21283, 0);
				}
		}
		InvalidateRectSafe(hwnd, NULL, TRUE);
		UpdateWindowSafe(hwnd);
